...
class StorageOverlayScreen : Screen(Component.literal("Storage Overlay")) {
    ...
    fun mouseScrolled(mouseX: Double, mouseY: Double, horizontalAmount: Double, verticalAmount: Double, activePage: StoragePageSlot?): Boolean {
        if (activePage != null && StorageOverlay.lockScrollOnActive) {
            ...
            layoutedForEach(StorageOverlay.storageData) { x, y, pageWidth, pageHeight, page, _ ->
                if (page == activePage && inRect(mouseX, mouseY, x, y, pageWidth, pageHeight)) {
                    overActive = true
                }
            }
    ...
    fun drawPages(context: GuiGraphics, mouseX: Int, mouseY: Int, delta: Float, excluding: StoragePageSlot?, slots: List<Slot>?) {
        createScissors(context)
        val viewTop = scrollPanelY
        val viewBottom = scrollPanelY + scrollPanelH
        layoutedForEach(StorageOverlay.storageData) { x, y, _, pageHeight, page, inventory ->
            if (y + pageHeight < viewTop || y > viewBottom) return@layoutedForEach
    ...
    fun getPlayerInventorySlotPosition(index: Int): Pair<Int, Int> {
        ...
        return if (index < 9) {
            Pair(baseX + index * SLOT_SIZE, baseY + 3 * SLOT_SIZE + 4)
        } else {
            Pair(baseX + (index % 9) * SLOT_SIZE, baseY + (index / 9 - 1) * SLOT_SIZE)
        }
    }
    ...
    private fun drawSlotGrid(context: GuiGraphics, x: Int, y: Int, rows: Int) {
        ...
        context.fill(x, y, x + width, y + height, slotCellBg)
        for (col in 0..9) {
            val lineX = x + col * SLOT_SIZE
            context.fill(lineX, y, lineX + 1, y + height, slotCellBorder)
        }
        for (row in 0..rows) {
            val lineY = y + row * SLOT_SIZE
            context.fill(x, lineY, x + width, lineY + 1, slotCellBorder)
        }
    }

    fun drawPlayerInventory(context: GuiGraphics, mouseX: Int, mouseY: Int) {
        val items = mc.player?.inventory?.nonEquipmentItems ?: return
        val (inventoryX, inventoryY) = getPlayerInventorySlotPosition(9)
        val (hotbarX, hotbarY) = getPlayerInventorySlotPosition(0)

        drawSlotGrid(context, inventoryX - 1, inventoryY - 1, 3)
        drawSlotGrid(context, hotbarX - 1, hotbarY - 1, 1)

        for (index in 0 until 36) {
            val item = items[index]
            if (item.isEmpty) continue
            val (slotX, slotY) = getPlayerInventorySlotPosition(index)
            if (StorageOverlayInventorySearchCompat.matches(item)) {
                Render2D.drawRect(context, slotX, slotY, 16, 16, StorageOverlayInventorySearchCompat.color())
            }
            context.renderItem(item, slotX, slotY, 0)
            context.renderItemDecorations(font, item, slotX, slotY)
        }
    }
    ...
    fun drawPage(context: GuiGraphics, x: Int, y: Int, page: StoragePageSlot, inventory: StorageData.StorageInventory, slots: List<Slot>?, mouseX: Int, mouseY: Int): Int {
        val virtualInventory = inventory.inventory
        if (virtualInventory == null && slots == null) {
        ...
        val rows = virtualInventory?.rows ?: (slots?.size?.div(9)?.coerceIn(1, 5) ?: 3)
        ...
        context.drawString(font, Component.literal(inventory.title), x + 6, y + 3, if (isActive) activePageBorder.rgb else 0xFFFFFF, true)
        ...
        val inventoryStacks = virtualInventory?.stacks
        val itemCount = inventoryStacks?.size ?: (slots?.size ?: (rows * 9))
        ...
        for (index in 0 until itemCount) {
            ...
            val displayStack = if (slots != null && index < slots.size) slots[index].item else inventoryStacks?.get(index) ?: continue
            if (displayStack.isEmpty) continue

            if (StorageOverlayInventorySearchCompat.matches(displayStack)) {
                Render2D.drawRect(context, slotX, slotY, 16, 16, StorageOverlayInventorySearchCompat.color())
            }
    ...
    private fun dispatchActivePageSlotClick(click: MouseButtonEvent, mouseX: Double, mouseY: Double, activePage: StoragePageSlot): Boolean {
        ...
        var hit = -1
        layoutedForEach(StorageOverlay.storageData) { x, y, _, _, page, inventory ->
            if (page != activePage) return@layoutedForEach
            val virtualInventory = inventory.inventory ?: return@layoutedForEach
            val gridX = x + 3
            val gridY = y + 5 + font.lineHeight + 1
            if (!inRect(mouseX, mouseY, gridX, gridY, 9 * SLOT_SIZE, virtualInventory.rows * SLOT_SIZE)) return@layoutedForEach
            val col = ((mouseX - gridX) / SLOT_SIZE).toInt().coerceIn(0, 8)
            val row = ((mouseY - gridY) / SLOT_SIZE).toInt().coerceIn(0, virtualInventory.rows - 1)
            hit = row * 9 + col
        }
        ...
        gameMode.handleInventoryMouseClick(menu.containerId, targetSlot.index, click.button(), clickType, player)
        return true
    }
    ...
    fun mouseClicked(click: MouseButtonEvent, doubled: Boolean, activePage: StoragePageSlot?): Boolean {
        ...
        if (inRect(mouseX, mouseY, scrollPanelX, scrollPanelY, scrollPanelW, scrollPanelH)) {
            ...
            layoutedForEach(StorageOverlay.storageData) { x, y, pageWidth, pageHeight, page, _ ->
                if (inRect(mouseX, mouseY, x, y, pageWidth, pageHeight) && activePage != page && click.button() == 0) {
                    page.navigateTo()
                    return true
                }
            }
