...
import com.github.noamm9.critsaddons.mixins.AccessorAbstractContainerScreenStorageOverlay
import com.github.noamm9.critsaddons.ui.customgui.CustomGui
import com.github.noamm9.critsaddons.ui.customgui.customGui
import com.github.noamm9.critsaddons.ui.customgui.setSlotX
import com.github.noamm9.critsaddons.ui.customgui.setSlotY
...
class StorageOverlayCustom(
    ...
    override fun onInit() {
        ...
        accessor.setImageWidthStorageOverlay(overview.measurements.overviewWidth)
        accessor.setImageHeightStorageOverlay(overview.measurements.totalHeight)
    }

    override fun shouldDrawForeground() = false

    override fun mouseClick(click: MouseButtonEvent, doubled: Boolean) =
        overview.mouseClicked(click, doubled, (handler as? StorageBackingHandle.Page)?.storagePageSlot)

    override fun mouseReleased(click: MouseButtonEvent) = overview.mouseReleased(click)

    override fun mouseDragged(click: MouseButtonEvent, deltaX: Double, deltaY: Double) =
        overview.mouseDragged(click, deltaX, deltaY)

    override fun mouseScrolled(mouseX: Double, mouseY: Double, horizontalAmount: Double, verticalAmount: Double) =
        overview.mouseScrolled(mouseX, mouseY, horizontalAmount, verticalAmount, (handler as? StorageBackingHandle.Page)?.storagePageSlot)
    ...
    override fun moveSlot(slot: Slot) {
        if (slot.container is Inventory) {
            ...
            val accessor = screen as AccessorAbstractContainerScreenStorageOverlay
            slot.setSlotX(x - accessor.getLeftPosStorageOverlay())
            slot.setSlotY(y - accessor.getTopPosStorageOverlay())
        } else {
...
object StorageOverlay : Feature(
    ...
    @Volatile
    private var handlingScreenChange = false

    override fun init() {
        register<ScreenChangeEvent> { onScreenChange(event) }
        register<WorldChangeEvent> { ensureDataLoaded() }
        register<TickEvent.Start> { currentHandler?.let(::rememberContent) }
    ...
    private fun onScreenChangeInner(event: ScreenChangeEvent) {
        ...
        val storageOverlayScreen = event.old as? StorageOverlayScreen
            ?: ((event.old as? AbstractContainerScreen<*>)?.customGui as? StorageOverlayCustom)?.overview
    ...
    fun rememberContent(handler: StorageBackingHandle?) {
        handler ?: return
        when (handler) {
            is StorageBackingHandle.Overview -> rememberStorageOverview(handler, storageData.storageInventories)
            is StorageBackingHandle.Page -> rememberPage(handler, storageData.storageInventories)
        }
    }
    ...
    private fun saveData() {
        ...
        for ((slot, inventory) in storageData.storageInventories) {
            val prefix = slot.index.toString()
            root.putString("${prefix}_title", inventory.title)
            inventory.inventory?.let { root.putString("${prefix}_inv", it.serialize()) }
        }
    ...
    private fun loadData() {
        ...
        for (i in 0 until 27) {
            ...
            val inventoryKey = "${i}_inv"
            if (!root.contains(titleKey)) continue
            val title = root.getString(titleKey).orElse("")
            if (title.isEmpty()) continue
            val slot = StoragePageSlot(i)
            val inventory = if (root.contains(inventoryKey)) VirtualInventory.deserialize(root.getString(inventoryKey).orElse("")) else null
